...
from flexgen.fused_moe import fused_moe
...
class TorchTensor:
    ...
    def load_split_from_np_file(self, filename, expert_id, shard_id = None, shard_size = None):
        self.load_split_from_np(np.load(filename), expert_id, shard_id, shard_size)
...
class TorchDevice:
    ...
    def moe_input_embed(self, inputs, w_token, pad_token_id, donate):
        # decompress weights
    ...
    def moe_output_embed(self, inputs, w_ln, w_token, donate,
    ...
    def mha(self, inputs, attention_mask, w_preln, w_q, w_k, w_v,
        ...
        hidden = F.layer_norm(inputs.data, (h,), weight=w_preln.data)
        ...
        value = F.layer_norm(value, (h,), weight=w_ln.data)
    ...
    def mha_gen(self, inputs, attention_mask, w_preln, w_q, w_k, w_v,
    ...
    def block_sparse_moe(self, inputs, gate, ws, w2s, topk, donate):
        # decompress weights
        if ws.device.device_type == DeviceType.COMPRESSED:
            ws = ws.device.decompress(ws)
            w2s = w2s.device.decompress(w2s)

        b, s, h = inputs.shape
        inputs.data = inputs.data.view(b * s, h)

        router_logits = F.linear(inputs.data, gate.data)
        out = fused_moe(inputs.data, ws.data, w2s.data, router_logits, topk, renormalize=True, inplace=True)
        out = out.view(b, s, h)
