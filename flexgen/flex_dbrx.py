"""
Usage:
python3 -m flexgen.flex_moe --model facebook/opt-1.3b --gpu-batch-size 32 --percent 100 0 100 0 100 0
"""
...
from flexgen.dbrx_config import DbrxConfig
...
def init_weight_list_moe(weight_specs, policy, env):
    dev_percents = [policy.w_disk_percent, policy.w_cpu_percent, policy.w_gpu_percent]
    dev_choices = [env.disk, env.cpu, env.gpu]
    ...
    for i in range(len(weight_specs)):
        ...
        if not compress:
            ...
            if DUMMY_WEIGHT not in filename:
                if "experts" in filename[0]:
                    for file in filename:
                        expert_id = int(file.split("/")[-1].split(".")[5])
                        if "w2" in file:
                            weight.dbrx_load_split_from_np_file(file, expert_id)
                        elif "w1" in file:
                            weight.dbrx_load_split_from_np_file(file, expert_id, 0, shape[1]//2)
                        elif "w3" in file:
                            weight.dbrx_load_split_from_np_file(file, expert_id, 1, shape[1]//2)
                else:
                    weight.load_from_np_file(weight_specs[i][2])
            else:
                weight.load_from_np(np.ones(shape, dtype))
...
class InputEmbed:
    ...
    def init_weight(self, weight_home, path):
        # NOTE: DBRX
        v, h, dtype = (self.config.vocab_size, self.config.ffn_config.ffn_hidden_size, self.config.dtype)
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.moe_input_embed(h, w_token, self.config.pad_token_id, donate)
        hidden.val = h
...
class OutputEmbed:
    ...
    def init_weight(self, weight_home, path):
        ...
        weight_specs = [
            ...
            # NOTE: DBRX 
            ((h,), dtype, path + "transformer.norm_f.weight")
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.moe_output_embed(h, w_ln, w_token, donate,
...
class SelfAttention:
    ...
    # NOTE: DBRX 
    def init_weight(self, weight_home, path):
        n_head, n_kv_head, h, dtype = (self.config.num_attention_heads, self.config.attn_config.kv_n_heads, 
                                       self.config.hidden_size, self.config.dtype)
        n_group = n_head // n_kv_head
        h_kv = h // n_group

        path_proj = os.path.join(path, f"transformer.blocks.{self.layer_id}.norm_attn_norm.attn")
        path_norm = os.path.join(path, f"transformer.blocks.{self.layer_id}.norm_attn_norm") # layernorm 
        
        weight_specs = [
            # w_preln
            ((h,), dtype, path_norm + ".norm_1.weight"),
            
            # w_q
            # ((h, h), dtype, path_proj + ".q_proj.weight"),
            # # w_k
            # ((h_kv, h), dtype, path_proj + ".k_proj.weight"),
            # # w_v
            # ((h_kv, h), dtype, path_proj + ".v_proj.weight"),
            
            # TODO: shape correct?
            # w_qkv 
            ((3 * h_kv, h), dtype, path_proj + ".Wqkv.weight"),
            
            # w_out
            ((h, h), dtype, path_proj + ".out_proj.weight"),
            
            # w_ln
            ((h,), dtype, path_norm + ".norm_2.weight"),
        ]
        weights = init_weight_list(weight_specs, self.policy, self.env)
        weight_home.store(weights)
    ...
    # Mixtral 
    # def init_weight(self, weight_home, path):
    #     n_head, n_kv_head, h, dtype = (self.config.num_attention_heads, self.config.num_key_value_heads, self.config.hidden_size, self.config.dtype)
    #     n_group = n_head // n_kv_head
    #     h_kv = h // n_group
    #     path_proj = os.path.join(os.path.join(path, f"model.layers.{self.layer_id}.self_attn"))
    #     path_postln = os.path.join(os.path.join(path, f"model.layers.{self.layer_id}.post_attention_layernorm"))
    #     path_preln = os.path.join(os.path.join(path, f"model.layers.{self.layer_id}.input_layernorm"))
    #     weight_specs = [
    #         # w_preln
    #         ((h,), dtype, path_preln + ".weight"),
    #         # w_q
    #         ((h, h), dtype, path_proj + ".q_proj.weight"),
    #         # w_k
    #         ((h_kv, h), dtype, path_proj + ".k_proj.weight"),
    ...
    #         ((h_kv, h), dtype, path_proj + ".v_proj.weight"),
    #         # w_out
    #         ((h, h), dtype, path_proj + ".o_proj.weight"),
    #         # w_ln
    #         ((h,), dtype, path_postln + ".weight"),
    #     ]
    #     weights = init_weight_list(weight_specs, self.policy, self.env)
    #     weight_home.store(weights)

    def load_weight(self, weight_home, weight_read_buf, k):
        w_preln, w_q, w_k, w_v, w_out, w_ln = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((
                w_preln.smart_copy(dst2),
                w_q.smart_copy(dst1),
                w_k.smart_copy(dst1),
                w_v.smart_copy(dst1),
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        n_kv_head = self.config.attn_config.kv_n_heads
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            ((w_preln, donate[11]), (w_q, donate[2]), (w_k, donate[4]),
             (w_v, donate[6]), (w_out, donate[8]),
             (w_ln, donate[10])) = weight_read_buf.pop()
        else:
            ((w_preln, _), (w_q, _), (w_k, _),
             (w_v, _), (w_out, _),
             (w_ln, _)) = weight_read_buf.val

        if i == 0:  # prefill
            mask, donate[1] = attention_mask.val.smart_copy(self.compute)
            h, new_k_cache, new_v_cache = self.compute.mha(h, mask, w_preln, w_q,
                w_k, w_v, w_out, w_ln, n_head, n_kv_head, donate,
                self.policy.compress_cache, self.policy.comp_cache_config)
            cache_write_buf.store((new_k_cache, new_v_cache))
        else:  # decoding
            mask, donate[1] = attention_mask.val.smart_copy(self.attention_compute)
            (k_cache, donate[12]), (v_cache, donate[13]) = cache_read_buf.pop()
            h, new_k_cache, new_v_cache = self.compute.mha_gen(h, mask, w_preln, w_q,
...
class MoEDBRXMLP:
    ...
    def init_weight(self, weight_home, path):
        ...
        # NOTE: Mixtral 
        # h, h2, n_experts, dtype = (self.config.hidden_size, self.config.intermediate_size, self.config.num_local_experts, self.config.dtype)
        # path = os.path.join(os.path.join(path, f"model.layers.{self.layer_id}.block_sparse_moe.")) 
        # weight_specs = [
        #     # gate
        #     ((n_experts, h), dtype, path + "gate.weight"), 
        #     # ws
        #     ((n_experts, 2*h2, h), dtype, [path + f"experts.{i}.w1.weight" for i in range(n_experts)]+[path + f"experts.{i}.w3.weight" for i in range(n_experts)]),              
        #     # w2s
        #     ((n_experts, h, h2), dtype, [path + f"experts.{i}.w2.weight" for i in range(n_experts)]),
        # ]
        
        # NOTE: DBRX 
        intermediate_size = (self.config.ffn_config.ffn_hidden_size // self.tp_size)
        h, h2, n_experts, dtype = (self.config.ffn_config.ffn_hidden_size, intermediate_size, 
                                   self.config.ffn_config.moe_num_experts, self.config.dtype)
        
        path = os.path.join(os.path.join(path, f"transformer.blocks.{self.layer_id}.ffn.")) 
        weight_specs = [
            # gate
            ((n_experts, h), dtype, path + "router.layer.weight"), 
            
            # ws
            ((n_experts, 2*h2, h), dtype, [path + f"experts.mlp.w1"]+[path + f"experts.mlp.v1"]), 
             
            # w2s
            ((n_experts, h, h2), dtype, [path + f"experts.mlp.w2"]), 
        ]
        
        weights = init_weight_list_moe(weight_specs, self.policy, self.env)
        weight_home.store(weights)

    def load_weight(self, weight_home, weight_read_buf, k):
        gate, ws, w2s = weight_home.val
        if k == 0:
            dst1 = self.weight_load_dst
            dst2 = self.compute
            weight_read_buf.store((
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        if k == self.policy.num_gpu_batches - 1:
            # Clear the weight_read_buf if it is the last gpu batch
            ((gate, donate[1]), (ws, donate[2]), (w2s, donate[3])) = weight_read_buf.pop()
        else:
            ((gate, _), (ws, _), (w2s, _)) = weight_read_buf.val

        h = self.compute.block_sparse_moe(h, gate, ws, w2s, self.config.ffn_config.moe_top_k, donate)
        hidden.val = h
...
class TransformerLayer:
    def __init__(self, config, env, policy, i):
        ...
        self.mlp = MoEDBRXMLP(config, env, policy, i)
        self.policy = policy
...
class MixtralLM:
    def __init__(self,
        ...
        for i in range(self.config.num_hidden_layers):
            if policy.sep_layer:
                ...
                layers.append(MoEDBRXMLP(self.config, self.env, self.policy, i))
            else:
    ...
    def init_weight(self, j):
        model_name_abbr = self.model_name.split("/")[-1].lower()
        expanded_path = os.path.abspath(os.path.expanduser(
            os.path.join(self.path, f"{model_name_abbr}-np")))
        check_path = os.path.join(expanded_path, "lm_head.weight")
        if not os.path.exists(check_path) and DUMMY_WEIGHT not in check_path:
            download_weights(self.model_name, self.path)
...
def model_bytes(config: DbrxConfig):
        h = config.hidden_size
        # h2 = config.intermediate_size
        # NOTE: dbrx -- shall we divide by the world size?
        h2 = (config.ffn_config.ffn_hidden_size // get_tensor_model_parallel_world_size())
        n = config.ffn_config.moe_num_experts
        n_kv_head = config.attn_config.kv_n_heads # config.num_key_value_heads 
        n_heads = config.num_attention_heads
        num_group = n_heads // n_kv_head
        return 	2 * (config.num_hidden_layers * (
        # self-attention
        h * 3 * h // num_group + h * (h + 1) +
        # gate
        h * n +
        # mlp
        3 * n * h * h2+
...
def cache_bytes(config: DbrxConfig, batch_size, seq_len):
    return 2 * batch_size * seq_len * config.num_hidden_layers * (config.hidden_size //config.num_attention_heads) * config.attn_config.kv_n_heads * 2

def hidden_bytes(config: DbrxConfig, batch_size, seq_len):
...
def run_flexgen(args):
    ...
    log_str = write_benchmark_log(filename,
        model_bytes(mixtral_config), cache_size, hidden_size,
