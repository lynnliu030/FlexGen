            ...
            if DUMMY_WEIGHT not in filename:
                if "experts" in filename[0]:
                    for file in filename:
                        ...
                        if "w2" in file:
                            weight.load_split_from_np_file(file, expert_id)
                        elif "w1" in file:
                            weight.load_split_from_np_file(file, expert_id, 0, shape[1]//2)
                        elif "w3" in file:
                            weight.load_split_from_np_file(file, expert_id, 1, shape[1]//2)
                else:
                    weight.load_from_np_file(weight_specs[i][2])
            else:
                weight.load_from_np(np.ones(shape, dtype))
...
class MoEMLP:
    ...
    def forward(self, hidden, cache_read_buf, weight_read_buf, attention_mask,
        ...
        h = self.compute.block_sparse_moe(h, gate, ws, w2s, self.config.num_experts_per_tok, donate)
        hidden.val = h
...
class TransformerLayer:
    def __init__(self, config, env, policy, i):
        ...
        self.mlp = MoEMLP(config, env, policy, i)
        self.policy = policy
...
def model_bytes(config: MixtralConfig):
